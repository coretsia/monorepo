...
The token MUST NOT be inserted into the handle context, reconstructed from the exported context, or exposed through any handle accessor.
...
`Stopwatch` itself remains strict: `Stopwatch::stop(int $startedAtToken)` accepts only a positive token returned by `Stopwatch::start()`.
