...
final class KernelConfigModuleStopwatchFailurePolicyContractTest extends TestCase
{
    public function testConfigKernelUsesSafeStopwatchWrappersOnly(): void
    {
        ...
        self::assertStringContainsString('$startedAt = $this->safeStartTimer();', $outsideWrappers);
        self::assertStringContainsString('$durationMs = $this->safeStopTimer($startedAt);', $outsideWrappers);

        self::assertStringNotContainsString(
            '->stopwatch->start(',
            $outsideWrappers,
            'ConfigKernel must access Stopwatch::start() only inside safeStartTimer().',
        );

        self::assertStringNotContainsString(
            '->stopwatch->stop(',
            $outsideWrappers,
            'ConfigKernel must access Stopwatch::stop() only inside safeStopTimer().',
        );
    }
    ...
    public function testModulePlanResolverUsesSafeStopwatchWrappersOnly(): void
    {
        ...
        self::assertStringNotContainsString(
            '->stopwatch->start(',
            $outsideWrappers,
            'ModulePlanResolver must access Stopwatch::start() only inside safeStartTimer().',
        );

        self::assertStringNotContainsString(
            '->stopwatch->stop(',
            $outsideWrappers,
            'ModulePlanResolver must access Stopwatch::stop() only inside safeStopTimer().',
        );
    }
    ...
    private static function withoutMethodBody(string $source, string $methodName): string
    {
        ...
        for ($i = $openBrace; $i < $length; $i++) {
            ...
            if ($source[$i] === '}') {
                ...
                if ($depth === 0) {
                    return \substr($source, 0, $openBrace + 1)
