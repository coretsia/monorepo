...
final class ToolsSpikesDoNotWriteToStdoutTest extends TestCase
{
    public function testToolsSpikesDoNotUseDirectStdoutStderrSinks(): void
    {
        ...
        foreach ($files as $file) {
            ...
            $this->scanTokens(
                $this->displayPath($repoRoot, $file),
                $tokens,
            );
        }
    ...
    private function scanTokens(string $displayFile, array $tokens): void
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($id === \T_ECHO) {
                $this->failToken($displayFile, $line, 'echo is forbidden in tools spikes.');
            }

            if ($id === \T_PRINT) {
                $this->failToken($displayFile, $line, 'print is forbidden in tools spikes.');
            }
            ...
            if ($isCall && \in_array($name, ['var_dump', 'print_r', 'printf', 'vprintf', 'fprintf'], true)) {
                $this->failToken($displayFile, $line, $name . '() is forbidden in tools spikes.');
            }
                    ...
                    if ($argIdx !== null) {
                        ...
                        if ($const === 'STDOUT' || $const === 'STDERR') {
                            $this->failToken(
                                $displayFile,
                                $line,
                                $name . '(' . $const . ', ...) is forbidden in tools spikes.'
                            );
                        }
                    }
                }
                    ...
                    if (
                        ...
                        if (
                            ...
                            $this->failToken(
                                $displayFile,
                                $line,
                                $name . '("php://stdout|stderr|output", ...) is forbidden in tools spikes.'
                            );
                        }
                    }
                }
