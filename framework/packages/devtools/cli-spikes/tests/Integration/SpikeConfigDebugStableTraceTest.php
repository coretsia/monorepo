...
final class SpikeConfigDebugStableTraceTest extends TestCase
{
    public function testSpikeConfigDebugIsDeterministicAndDoesNotLeakPaths(): void
    {
        ...
        self::assertTrue(
            \class_exists('Coretsia\\Tools\\Spikes\\config_merge\\ConfigMerger'),
            'Config merge spike MUST exist (prerequisite 0.90.0).'
        );
        ...
        $this->withLauncherAtRepoRoot($repoRoot, function () use (&$payload1, &$payload2): void {
            ...
            self::assertSame(0, $exit1, 'spike:config:debug MUST succeed.');
            ...
            self::assertSame(0, $exit2, 'spike:config:debug MUST be rerunnable and succeed.');
        });

        self::assertIsArray($payload1);
        self::assertIsArray($payload2);

        self::assertSame($payload1, $payload2, 'config debug trace MUST be deterministic across two runs.');

        self::assertSame(1, $payload1['schema_version'] ?? null);
        self::assertSame('spike:config:debug', $payload1['command'] ?? null);
        self::assertSame('cli.commands', $payload1['key'] ?? null);

        self::assertIsArray($payload1['resolved'] ?? null, 'resolved MUST be present.');
        self::assertIsArray($payload1['trace'] ?? null, 'trace MUST be present.');
