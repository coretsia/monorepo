...
(static function (): void {
    ...
    $fallbackViolation = 'CORETSIA_NO_SKELETON_HTTP_DEFAULT_FORBIDDEN';
    $fallbackScanFailed = 'CORETSIA_NO_SKELETON_HTTP_DEFAULT_GATE_FAILED';
    ...
    if (!\is_file($bootstrap) || !\is_readable($bootstrap)) {
        if (\is_file($consoleFile) && \is_readable($consoleFile)) {
            ...
            if (\is_file($errorCodesFile) && \is_readable($errorCodesFile)) {
                ...
                $name = $ErrorCodes . '::CORETSIA_NO_SKELETON_HTTP_DEFAULT_GATE_FAILED';
                if (\defined($name)) {
                    /** @var string $v */
                    $v = \constant($name);
                    $code = $v;
    ...
    $codeViolation = (static function () use ($ErrorCodes, $fallbackViolation): string {
        $name = $ErrorCodes . '::CORETSIA_NO_SKELETON_HTTP_DEFAULT_FORBIDDEN';
        if (\defined($name)) {
            /** @var string $v */
            $v = \constant($name);
            return $v;
    ...
    try {
        ...
        $forbiddenFile = $repoRoot . '/skeleton/config/http.php';

        /** @var list<string> $violations */
        $violations = [];

        if (\is_file($forbiddenFile)) {
            $violations[] = coretsia_no_skeleton_http_default_gate_rel_from_repo($forbiddenFile, $repoRoot)
                . ': forbidden-default-http-config';
        }
...
function coretsia_no_skeleton_http_default_gate_rel_from_repo(string $absPath, string $repoRoot): string
{
