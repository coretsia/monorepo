...
{
    public const string CODE_FAILED = 'CORETSIA_RELEASE_LINE_PUBLIC_CONSTRAINTS_SYNC_FAILED';
    private const string CODE_OUT_OF_SYNC = 'CORETSIA_RELEASE_LINE_PUBLIC_CONSTRAINTS_OUT_OF_SYNC';
    ...
    public static function main(array $argv): int
    {
        ...
        $packages = self::discoverWorkspacePackages($repoRoot);

        $changedFiles = [];

        foreach ($packages as $package) {
            $changed = self::syncPackageComposer(
                $package['path'],
            ...
            if ($changed) {
                $changedFiles[] = self::rel($repoRoot, $package['path']);
            }
        }
    ...
    /**
     * @return list<array{path:string,packageId:string,name:string}>
     */
    private static function discoverWorkspacePackages(string $repoRoot): array
    {
    ...
    private static function syncPackageComposer(
        ...
        $originalBytes = self::readFile($composerJsonPath, $repoRoot);
        ...
        if (!is_array($data) || array_is_list($data)) {
            throw new RuntimeException('Invalid JSON object: ' . self::rel($repoRoot, $composerJsonPath));
        }
        ...
        if ($newJson === self::normalizeToLfFinalNewline($bytesForJsonDecode)) {
            return false;
        }
    ...
    private static function writeBackupIfNeeded(string $composerJsonPath, string $originalBytes, string $repoRoot): void
    {
        ...
        $relativeDir = dirname(self::rel($repoRoot, $composerJsonPath));
     ...
     * - NO global key sorting/reordering; only already-mutated dependency values are changed
     */
...
try {
    ...
    $msg = str_replace(["\r\n", "\r"], "\n", $e->getMessage());
    fwrite(STDERR, SyncPackagePublicConstraints::CODE_FAILED . ": {$msg}\n");
    exit(1);
}
