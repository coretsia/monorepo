...
final readonly class ConfigValueSource
{
     ...
     * @param non-empty-string|null $path Relative source path or logical source path. Absolute paths are forbidden.
    ...
    public function __construct(
        ...
        $this->path = self::normalizeOptionalRelativeSourcePath($path);
     ...
     * @return non-empty-string|null
     */
    private static function normalizeOptionalRelativeSourcePath(?string $path): ?string
    {
        if ($path === null) {
            return null;
        }
        ...
        if (str_contains($path, ':')) {
            throw new \InvalidArgumentException('Config value source path must be a safe relative source path.');
        }
