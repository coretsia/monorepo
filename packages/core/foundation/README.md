...
`core/foundation` is the Foundation runtime package for Coretsia.
...
Out of scope: kernel lifecycle execution, HTTP middleware stack implementation, CLI command execution, platform adapters, integrations, HTTP correlation header extraction/injection policy, logs/traces/metrics exporters, developer tooling, and repository automation.
...
`core/foundation` MUST NOT depend on `devtools/*` packages and MUST NOT import or consume repository machinery under `tools/**`.
...
`Coretsia\Foundation\Tag\ReservedTags` is the canonical code-level registry for Coretsia-reserved DI tag identifiers.
...
- [Foundation package source](https://github.com/coretsia/monorepo/tree/main/packages/core/foundation)
