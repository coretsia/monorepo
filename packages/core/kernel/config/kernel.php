 ...
 * - Kernel config defaults MUST NOT contain repository-topology-dependent paths;
...
return [
     ...
     * Coretsia implementation itself.
     *
     * Important:
     *
     * - `kernel` MUST NOT be listed here;
     * - `foundation` MUST NOT be listed here;
     * - applications must remain able to configure package-owned public roots
     ...
     *
     * Per-app preset overrides are application-local bootstrap-only policy and may be
     * defined in config/app.php under the `presets` key.
     *
     ...
     *
     * `default_artifacts_cache_dir` is BootstrapConfig::applicationRoot()-relative.
     * It MUST remain a relative-safe path and MUST NOT contain `..`, absolute path
     * syntax, host-specific path fragments, or repository-topology-dependent paths.
     *
     ...
     * package root.
     *
     * `overrides_path` is application-root-relative and resolves from
     * BootstrapConfig::applicationRoot().
     *
     * These defaults intentionally avoid absolute paths and repository-topology-dependent
     ...
     * bytes.
     *
     * `application_ignore_prefixes` values are BootstrapConfig::applicationRoot()
     * relative. They are used only as deterministic exclusions for application-local
     ...
     *
     * This section owns only stable shape limits for context attributes.
     * Lifecycle execution, HTTP/CLI outcome derivation, reset triggering, and
     * adapter-specific attribute population are owned by their runtime/platform layers.
     */
    'uow' => [
        /*
