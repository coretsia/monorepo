 ...
 * - per-app preset overrides belong to bootstrap-only `config/app.php`
 ...
 * - `kernel.fingerprint.application_ignore_prefixes` must be a list of non-empty
 *   relative safe paths;
 ...
 * - `worker.*` is not defined here because the `worker` root is owned by
 *   `platform/worker`;
 * - these rules define no reset tag constants, CLI adapter config, outcome
