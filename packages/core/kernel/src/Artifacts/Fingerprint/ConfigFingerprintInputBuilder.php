...
final readonly class ConfigFingerprintInputBuilder
{
    ...
    private const string KERNEL_KEY_APPLICATION_IGNORE_PREFIXES = 'application_ignore_prefixes';
    ...
    private const string SOURCE_KIND_APPLICATION_CONFIG = 'application_config';
    ...
    public function build(
        ...
        $applicationRoot = self::normalizeApplicationRoot($bootstrapConfig->applicationRoot());
        $configuredApplicationIgnorePrefixes = self::applicationIgnorePrefixes(
            $kernelConfig,
        );

        $effectiveApplicationIgnorePrefixes = self::mergeApplicationIgnorePrefixes(
            configuredPrefixes: $configuredApplicationIgnorePrefixes,
            mandatoryPrefixes: [
                $bootstrapConfig->artifactsCacheDir(),
            ],
        );

        $applicationConfigSources = self::compiledSourceFileList(
        ...
        $packageConfigCandidates = $this->sourceCandidateList(
            ...
            applicationIgnorePrefixes: $effectiveApplicationIgnorePrefixes,
        );
        ...
        $input = [
            ...
            'fingerprintPolicy' => [
                'applicationIgnorePrefixes' => $configuredApplicationIgnorePrefixes,
            ],
            ...
            'sourceCandidates' => [
                self::SOURCE_KIND_APPLICATION_CONFIG => $applicationConfigSources,
            ...
            'observabilityMetadata' => [
                ...
                'sourceCandidateCounts' => [
                    self::SOURCE_KIND_APPLICATION_CONFIG => \count($applicationConfigSources),
                ...
                'missingCandidateCounts' => [
                    self::SOURCE_KIND_APPLICATION_CONFIG => self::missingCandidateCount($applicationConfigSources),
     ...
     * @param list<non-empty-string> $applicationIgnorePrefixes
     *
     * @return list<array<string, mixed>>
     */
    private function sourceCandidateList(
        ...
        array $applicationIgnorePrefixes,
    ): array {
        $out = [];
        ...
        if (@\is_dir($filesystemPath)) {
            ...
            foreach (
                $this->fileLister->listFiles(
                    ...
                    skipRelativePath: self::applicationIgnoreCallback(
                        declaredRoot: $filesystemPath,
                        applicationRoot: $applicationRoot,
                        applicationIgnorePrefixes: $applicationIgnorePrefixes,
                    ),
     ...
     */
    private static function applicationIgnoreCallback(
        string $declaredRoot,
        string $applicationRoot,
        array $applicationIgnorePrefixes,
    ): ?\Closure {
        if ($applicationIgnorePrefixes === []) {
            return null;
        }

        $normalizedApplicationRoot = self::normalizeApplicationRoot($applicationRoot);
        $normalizedDeclaredRoot = self::normalizeApplicationRoot($declaredRoot);

        if ($normalizedDeclaredRoot === $normalizedApplicationRoot) {
            $declaredRootApplicationRelative = '';
        } else {
            $declaredRootApplicationRelative = self::applicationRelativePathOrNull(
                applicationRoot: $normalizedApplicationRoot,
                filesystemPath: $normalizedDeclaredRoot,
            );

            if ($declaredRootApplicationRelative === null) {
                return null;
            }
        }
        ...
        return static function (string $candidateRelativePath) use (
            $declaredRootApplicationRelative,
            $applicationIgnorePrefixes,
        ): bool {
            $normalizedCandidateRelativePath = \str_replace('\\', '/', $candidateRelativePath);

            $candidateApplicationRelativePath = $declaredRootApplicationRelative === ''
                ? $normalizedCandidateRelativePath
                : $declaredRootApplicationRelative . '/' . $normalizedCandidateRelativePath;

            return self::isIgnoredApplicationRelativePath(
                relativePath: $candidateApplicationRelativePath,
                prefixes: $applicationIgnorePrefixes,
            );
        };
    }
     ...
     * @return list<non-empty-string>
     */
    private static function applicationIgnorePrefixes(array $kernelConfig): array
    {
        $value = $kernelConfig[self::KERNEL_KEY_FINGERPRINT][self::KERNEL_KEY_APPLICATION_IGNORE_PREFIXES] ?? [];

        if (!\is_array($value) || !\array_is_list($value)) {
            throw new \InvalidArgumentException('fingerprint-application-ignore-prefixes-invalid');
        }
        ...
        foreach ($value as $prefix) {
            if (!\is_string($prefix)) {
                throw new \InvalidArgumentException('fingerprint-application-ignore-prefix-invalid');
            }

            $normalized = self::normalizeFingerprintRelativePath(
                value: $prefix,
                reason: 'fingerprint-application-ignore-prefix-invalid',
            );
     ...
     * @return list<non-empty-string>
     */
    private static function mergeApplicationIgnorePrefixes(
     ...
     * @return non-empty-string
     */
    private static function normalizeApplicationRoot(string $applicationRoot): string
    {
        $normalized = \str_replace('\\', '/', $applicationRoot);
    ...
    private static function applicationRelativePathOrNull(
        string $applicationRoot,
        string $filesystemPath,
    ): ?string {
        $root = self::normalizeApplicationRoot($applicationRoot);
     ...
     */
    private static function isIgnoredApplicationRelativePath(
        string $relativePath,
        array $prefixes,
    ): bool {
    ...
    private function dotenvCandidateList(BootstrapConfig $bootstrapConfig, array $kernelConfig): array
    {
        ...
        foreach (self::dotenvFileNames($kernelConfig, $bootstrapConfig->appEnv()) as $fileName) {
            $filesystemPath = self::joinPath($bootstrapConfig->applicationRoot(), $fileName);
