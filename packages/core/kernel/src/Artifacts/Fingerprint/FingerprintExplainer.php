...
final readonly class FingerprintExplainer
{
    ...
    private static function collectFingerprintPolicyEntries(
        ...
        $prefixes = $fingerprintPolicy['applicationIgnorePrefixes'] ?? null;

        if (!\is_array($prefixes) || !\array_is_list($prefixes)) {
            $entries[] = self::entry([
                'kind' => 'fingerprint_policy',
                'sourceType' => 'application_ignore_prefixes',
                'reason' => self::REASON_INVALID,
            ]);
        ...
        foreach ($prefixes as $index => $prefix) {
            if ($index > self::MAX_SAFE_COUNT) {
                $entries[] = self::entry([
                    'kind' => 'fingerprint_policy',
                    'sourceType' => 'application_ignore_prefix',
                    'reason' => self::REASON_INVALID,
                ]);
            ...
            if (!\is_string($prefix)) {
                $entries[] = self::entry([
                    'kind' => 'fingerprint_policy',
                    'keyPath' => 'fingerprintPolicy.applicationIgnorePrefixes[' . $index . ']',
                    'sourceType' => 'application_ignore_prefix',
                    'reason' => self::REASON_INVALID,
                ]);
