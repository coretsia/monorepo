 ...
 * - load application-root/app-target config through ApplicationConfigLoader;
...
final readonly class ConfigKernel
{
    ...
    public function __construct(
        ...
        private ApplicationConfigLoader $applicationLoader,
    ...
    private function compileWithoutExplain(
        ...
        $applicationConfig = $this->applicationLoader->load(
            bootstrapConfig: $bootstrapConfig,
        ...
        return [
            ...
            'configSourceFiles' => $applicationConfig['sourceFiles'],
    ...
    private static function buildMergeEntries(
        ...
        $applicationSourcesByEntry = self::applicationSourcesByEntry($applicationConfig['sources']);

        foreach ($applicationConfig['entries'] as $entry) {
            $type = self::sourceTypeFromString($entry['type']);
            $sourceId = $entry['sourceId'];
            $pathSources = $applicationSourcesByEntry[$sourceId] ?? [];
     ...
     */
    private static function applicationSourcesByEntry(array $sources): array
    {
        $byEntry = [];
