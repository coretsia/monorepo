...
/**
 * Deterministic application config loader.
 *
 * This loader owns loading declared Phase B application-root/app-target config locations:
 *
 * - application-root shared aggregate root-map config:
 *   config/roots.php
 * - application-root shared split root-subtree config:
 *   config/<root>.php
 * - application-root environment aggregate root-map config:
 *   config/environments/<appEnv>/roots.php
 * - application-root environment split root-subtree config:
 *   config/environments/<appEnv>/<root>.php
 * - app-target shared aggregate root-map config:
 *   apps/<appTarget>/config/roots.php
 * - app-target shared split root-subtree config:
 *   apps/<appTarget>/config/<root>.php
 * - app-target environment aggregate root-map config:
 *   apps/<appTarget>/config/environments/<appEnv>/roots.php
 * - app-target environment split root-subtree config:
 ...
 *
 * The loader MUST NOT read `config/app.php`; that file is Bootstrap
 * Phase A input only.
 *
...
final readonly class ApplicationConfigLoader
{
    ...
    private const string LAYER_APPLICATION_SHARED = 'application_shared';
    private const string LAYER_APPLICATION_ENVIRONMENT = 'application_environment';
    private const string LAYER_APP_TARGET_SHARED = 'app_target_shared';
    private const string LAYER_APP_TARGET_ENVIRONMENT = 'app_target_environment';
    ...
    private const int PRECEDENCE_APPLICATION_SHARED_AGGREGATE = 100;
    private const int PRECEDENCE_APPLICATION_SHARED_ROOT = 101;
    private const int PRECEDENCE_APPLICATION_ENVIRONMENT_AGGREGATE = 200;
    private const int PRECEDENCE_APPLICATION_ENVIRONMENT_ROOT = 201;
    private const int PRECEDENCE_APP_TARGET_SHARED_AGGREGATE = 300;
    private const int PRECEDENCE_APP_TARGET_SHARED_ROOT = 301;
    private const int PRECEDENCE_APP_TARGET_ENVIRONMENT_AGGREGATE = 400;
    private const int PRECEDENCE_APP_TARGET_ENVIRONMENT_ROOT = 401;
    ...
    /**
     * Loads declared application-root/app-target config entries in deterministic
     * Phase B order.
     *
    ...
    public function load(
        ...
        $applicationRoot = $bootstrapConfig->applicationRoot();
        ...
        $this->loadLayer(
            collector: $collector,
            filesystemBase: $applicationRoot,
            configRelativeDirectory: self::CONFIG_DIRECTORY,
            layer: self::LAYER_APPLICATION_SHARED,
            type: ConfigSourceType::ApplicationConfig,
            appEnv: $appEnv,
            appTarget: $appTarget,
            declaredSplitRoots: $declaredSplitRoots,
            aggregatePrecedence: self::PRECEDENCE_APPLICATION_SHARED_AGGREGATE,
            rootPrecedence: self::PRECEDENCE_APPLICATION_SHARED_ROOT,
        );
        ...
        $this->loadLayer(
            ...
            layer: self::LAYER_APPLICATION_ENVIRONMENT,
            type: ConfigSourceType::ApplicationConfig,
            appEnv: $appEnv,
            appTarget: $appTarget,
            declaredSplitRoots: $declaredSplitRoots,
            aggregatePrecedence: self::PRECEDENCE_APPLICATION_ENVIRONMENT_AGGREGATE,
            rootPrecedence: self::PRECEDENCE_APPLICATION_ENVIRONMENT_ROOT,
        );
        ...
        $this->loadLayer(
            ...
            layer: self::LAYER_APP_TARGET_SHARED,
            type: ConfigSourceType::AppConfig,
            appEnv: $appEnv,
            appTarget: $appTarget,
            declaredSplitRoots: $declaredSplitRoots,
            aggregatePrecedence: self::PRECEDENCE_APP_TARGET_SHARED_AGGREGATE,
            rootPrecedence: self::PRECEDENCE_APP_TARGET_SHARED_ROOT,
        );
        ...
        $this->loadLayer(
            ...
            layer: self::LAYER_APP_TARGET_ENVIRONMENT,
            type: ConfigSourceType::AppConfig,
            appEnv: $appEnv,
            appTarget: $appTarget,
            declaredSplitRoots: $declaredSplitRoots,
            aggregatePrecedence: self::PRECEDENCE_APP_TARGET_ENVIRONMENT_AGGREGATE,
            rootPrecedence: self::PRECEDENCE_APP_TARGET_ENVIRONMENT_ROOT,
        );
    ...
    private function loadLayer(
        ...
        $logicalDirectory = $configRelativeDirectory;
    ...
    private static function sourceId(
        ...
        return 'application-config/' . $layer . '/' . $kind . '/' . \str_replace('/', '_', $path);
    }
