...
final class KernelServiceFactory
{
     ...
     * config/app.php and keeps no mutable runtime state.
     */
     ...
     * read config/app.php, read package defaults, parse dotenv files,read
     * system env, or keep mutable runtime state.
     */
     ...
     * resolve application override paths, or cache loaded presets.
     */
    ...
    /**
     * Creates the application config loader.
     *
     * This factory wires the loader to DirectiveProcessor. It does not scan
     * application-root/app-target config directories, does not read config/app.php,
     * and keeps no mutable runtime state.
     */
    public static function applicationConfigLoader(ContainerInterface $container): ApplicationConfigLoader
    {
        $directiveProcessor = self::configService($container, DirectiveProcessor::class);
    ...
    public static function configKernel(ContainerInterface $container): ConfigKernel
    {
        ...
        $applicationLoader = self::configService($container, ApplicationConfigLoader::class);

        if (!$applicationLoader instanceof ApplicationConfigLoader) {
            throw new ContainerException('kernel-config-dependency-invalid');
        }
