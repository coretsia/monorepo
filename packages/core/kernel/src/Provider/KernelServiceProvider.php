 ...
 * - registering ConfigKernel services does not load application-root/app-target config files;
 ...
 * - FilesystemModePresetLoader is not registered globally because application
 ...
 *
 * This provider must not emit stdout/stderr, must not use `devtools/*` packages
 * or consume repository machinery under `tools/**`, must not introduce static
 * mutable snapshots, must not trigger reset orchestration, must not execute
 * Bootstrap Phase A, must not inspect runtime driver config, must not run runtime
 * driver detection, must not resolve a ModulePlan, must not compile config, and
 * must not start a UnitOfWork during registration.
 */
    ...
    ContainerDefinitionProviderInterface
{
    ...
    public function register(ContainerBuilder $builder): void
    {
         ...
         * do not load config/app.php, do not parse dotenv files, do not
        ...
        $builder->factory(
            ...
            static fn (Container $_container): ModuleIdSetNormalizer => KernelServiceFactory::moduleIdSetNormalizer(),
        );
        $builder->factory(
            PresetNamespaceResolver::class,
            static fn (
                Container $_container,
            ): PresetNamespaceResolver => KernelServiceFactory::presetNamespaceResolver(),
        );
        $builder->factory(
            ModuleSelectionFactory::class,
            static fn (Container $container): ModuleSelectionFactory => KernelServiceFactory::moduleSelectionFactory(
                $container,
            ),
        );
        ...
        $builder->factory(
            ...
            static fn (
                Container $container,
            ): ModuleResolutionOrchestrator => KernelServiceFactory::moduleResolutionOrchestrator($container),
        );
         ...
         * EnvRepositoryInterface snapshots, do not load package defaults or
         * application-root/app-target config files, do not build env overlays, do not
         * merge config, do not validate config, and do not build explain traces
        ...
        $builder->factory(
            ...
            static fn (
                Container $container,
            ): ApplicationConfigLoader => KernelServiceFactory::ApplicationConfigLoader(
                container: $container,
            ),
        );
