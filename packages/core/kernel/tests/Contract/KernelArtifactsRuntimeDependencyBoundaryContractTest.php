...
final class KernelArtifactsRuntimeDependencyBoundaryContractTest extends TestCase
{
    ...
    public function testArtifactFingerprintAndCacheRuntimeCodeDoesNotReadToolsTree(): void
    {
        foreach (self::artifactRuntimeFiles() as $path) {
            ...
            self::assertStringNotContainsString(
                'tools/',
                $source,
                $relativePath . ' must not read or reference tools.',
            );
        }
    }

    public function testKernelTestsDoNotReferenceTools(): void
    {
