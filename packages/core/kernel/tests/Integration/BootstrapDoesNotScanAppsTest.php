...
final class BootstrapDoesNotScanAppsTest extends TestCase
{
    ...
    {
        $applicationRoot = self::createApplicationRoot('multiple-synthetic-app-dirs');

        try {
            self::createSyntheticAppDirs($applicationRoot, [
                'web',
                'api',
                'console',
                'worker',
            ...
            self::assertSame($applicationRoot . '/apps/console', $config->appRoot());
        } finally {
    ...
    {
        $applicationRoot = self::createApplicationRoot('sibling-presence-does-not-affect-result');
        ...
        try {
            ...
            self::assertSame($applicationRoot . '/apps/web', $beforeAddingMoreSiblings->appRoot());
            ...
            self::assertSame($applicationRoot . '/apps/web', $afterAddingMoreSiblings->appRoot());

            self::removeDirectory($applicationRoot . '/apps/api');
            self::removeDirectory($applicationRoot . '/apps/worker');
            self::removeDirectory($applicationRoot . '/apps/console');
            self::removeDirectory($applicationRoot . '/apps/admin');
            self::removeDirectory($applicationRoot . '/apps/legacy');
            self::removeDirectory($applicationRoot . '/apps/zzz');
            ...
            self::assertSame($applicationRoot . '/apps/web', $afterRemovingSiblings->appRoot());
        } finally {
            self::removeDirectory($applicationRoot);
        }
    }
     ...
     */
    private static function createSyntheticAppDirs(string $applicationRoot, array $appNames): void
    {
        foreach ($appNames as $appName) {
            self::ensureDirectory($applicationRoot . '/apps/' . $appName);
            self::ensureDirectory($applicationRoot . '/apps/' . $appName . '/config');

            $marker = $applicationRoot . '/apps/' . $appName . '/config/phase-a-must-not-read.php';
