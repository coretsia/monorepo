...
final class BootstrapSelectsExplicitAppTargetTest extends TestCase
{
    ...
    public function testBootstrapAcceptsExplicitAppTargetAndDerivesAppRoot(
        ...
        $applicationRoot = self::nonExistingApplicationRoot('accepted-' . $targetValue);

        self::assertDirectoryDoesNotExist($applicationRoot);
        ...
        self::assertSame(
            $applicationRoot . '/apps/' . $targetValue,
            $config->appRoot(),
        );
    }

    public function testBootstrapDoesNotScanAppsDirectory(): void
    {
        $applicationRoot = self::nonExistingApplicationRoot('no-scan');
        ...
        self::assertSame($applicationRoot . '/apps/web', $config->appRoot());
    }
     ...
     *
     * The test intentionally uses a missing application root to prove Bootstrap
     * Phase A target selection does not scan or require applicationRoot/apps/<app>.
     */
    private static function nonExistingApplicationRoot(string $case): string
    {
        $root = self::kernelRoot()
            . '/build/test-non-existing-application/bootstrap-selects-explicit-app-target/'
            . $case;
