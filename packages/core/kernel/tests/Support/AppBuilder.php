...
final class AppBuilder
{
    ...
    {
        $applicationRoot = self::temporaryApplicationRoot('boot-micro');

        self::assertCanonicalApplicationModeOverrideAbsent($applicationRoot, self::PRESET_MICRO);
        self::assertKernelDefaultPresetFileExists(self::PRESET_MICRO);
    ...
    public static function bootExpressExpectingRequiredMissing(TestCase $_testCase): AppBuilderRequiredMissingResult
    {
        ...
        self::assertCanonicalApplicationModeOverrideAbsent($applicationRoot, self::PRESET_EXPRESS);
        self::assertKernelDefaultPresetFileExists(self::PRESET_EXPRESS);
    ...
    public static function resolveApplicationOnlyPreset(
        ...
        $applicationRoot = self::temporaryApplicationRoot('application-only-preset');

        if (\is_file(self::kernelModeFile($presetName))) {
            throw new \LogicException('app-builder-kernel-custom-preset-unexpectedly-present');
        }

        ArtifactPipelineTestSupport::writePhpReturn(
            $applicationRoot . '/config/modes/' . $presetName . '.php',
            [
                'schemaVersion' => 1,
                'name' => $presetName,
     ...
     */
    public static function artifactPaths(string $applicationRoot): array
    {
        return ArtifactPipelineTestSupport::currentArtifactPaths($applicationRoot);
    }

    private static function temporaryApplicationRoot(string $name): string
    {
    ...
    private static function assertCanonicalApplicationModeOverrideAbsent(
        string $applicationRoot,
        string $presetName,
    ): void {
        if (\is_file($applicationRoot . '/config/modes/' . $presetName . '.php')) {
            throw new \LogicException('app-builder-canonical-application-mode-override-present');
        }
    }

    private static function assertKernelDefaultPresetFileExists(string $presetName): void
    {
        if (!\is_file(self::kernelModeFile($presetName))) {
            throw new \LogicException('app-builder-kernel-default-preset-missing');
        }
    }

    private static function kernelModeFile(string $presetName): string
    {
