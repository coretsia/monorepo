...
/**
 * Minimal deterministic CLI runtime.
 *
 * Responsibilities:
 * - deterministic config loading + merge (defaults → application override)
 ...
 * - $applicationRoot MUST be supplied explicitly by the entrypoint.
 * - Runtime CLI code MUST NOT derive application state from repository topology.
 *
 * @internal
 */
final class Application
{
    private const string APPLICATION_OVERRIDE_RELATIVE_FILE = '/config/cli.php';
     ...
     * - package defaults from config/cli.php
     * - optional application override resolved by the current standalone implementation
     *
    ...
    private function loadCliConfigSubtree(): array
    {
        ...
        $applicationOverride = $this->loadApplicationOverrideSubtree();

        // Merge order is single-choice and deterministic: defaults → application override.
        $cli = $defaults;
        $cli = $this->mergeCliSubtrees($cli, $applicationOverride);
    ...
    /**
     * Application override subtree.
     *
     * Policy:
     * - the application override root MAY be absent (treated as empty overlay)
     * - its config/cli.php MAY be absent (treated as empty overlay)
     * - if the file exists but is unreadable OR returns an invalid subtree => deterministic config invalid
     *
     * @return array<string, mixed>
     */
    private function loadApplicationOverrideSubtree(): array
    {
        ...
        $file = $applicationRoot . self::APPLICATION_OVERRIDE_RELATIVE_FILE;
     ...
     * We intentionally allow partial subtrees for overlays,
     ...
     *
     * Instantiation policy (single-choice):
    ...
    private function instantiateZeroArg(string $fqcn): CommandInterface
    {
        ...
        try {
            ...
            // Deterministic policy: ctor must be zero-arg and safe.
