...
final class CliConfigSubtreeShapeAndMergeSemanticsTest extends TestCase
{
    public function testConfigCliPhpReturnsSubtreeAndDoesNotRepeatRootKey(): void
    {
        ...
        $repoRoot = \dirname($packageRoot, 3);
        ...
        $templateFile = $repoRoot . '/packages/applications/skeleton/config/cli.php';
        if (\is_file($templateFile)) {
            $tpl = require $templateFile;
            self::assertIsArray($tpl, 'packages/applications/skeleton/config/cli.php (if present) MUST return the cli subtree array.');
            self::assertArrayNotHasKey('cli', $tpl, 'packages/applications/skeleton/config/cli.php MUST NOT repeat root key "cli".');
        }
    ...
    {
        $app = new Application('application-root-placeholder');

        $ref = new \ReflectionObject($app);
        $merge = $ref->getMethod('mergeCliSubtrees');

        // Defaults → application override (cemented order)
        $defaults = [
