...
exit(GateRuntime::execute(
    'CORETSIA_CROSS_CUTTING_CONTRACT_DRIFT',
    ...
    static function (RepositoryContext $repository): array {
        ...
        $contracts = $catalog->byPackageId('core/contracts');
        $foundation = $catalog->byPackageId('core/foundation');

        if ($contracts === null || $foundation === null) {
            return [];
        }

        $resetInterfaceFile = $contracts['absolutePath'] . '/src/Runtime/ResetInterface.php';
        $reservedTagsFile = $foundation['absolutePath'] . '/src/Tag/ReservedTags.php';

        if (!is_file($resetInterfaceFile) || !is_file($reservedTagsFile)) {
            return [];
        }

        $repository->resolveExistingFile($resetInterfaceFile);
        $reservedTagsFile = $repository->resolveExistingFile($reservedTagsFile);
        ...
        if (
            ...
            return [
                $repository->relativeToRepo($reservedTagsFile) . ': kernel-tags-drift',
            ];
        }
        ...
        [
            ...
            $repository,
            $foundation['absolutePath'],
            $defaultResetTag,
        );

        if ($effectiveResetTagViolations !== []) {
            return $effectiveResetTagViolations;
        }
        ...
        $sourceRoots = coretsia_cross_cutting_contract_gate_product_roots(
            $repository,
            $catalog,
            ['src'],
        );
        ...
        $wiringRoots = coretsia_cross_cutting_contract_gate_product_roots(
            $repository,
            $catalog,
            ['src', 'config'],
        );
        ...
        foreach (
            ...
            foreach (
                ...
                if (!$evidence['has_reset_tag']) {
                    $violations[] = $repoRelPath . ': kernel-stateful-service-missing-reset-tag';
                }

                if ($evidence['service_classes'] === []) {
                    $violations[] = $repoRelPath . ': kernel-stateful-service-class-unresolved';
                    continue;
                }

                foreach ($evidence['service_classes'] as $serviceClass) {
                    if (!isset($resettableClasses[$serviceClass])) {
                        $violations[] = $repoRelPath . ': kernel-stateful-service-not-resettable';
                    }
                }
            }
 ...
 * @return list<string>
 */
function coretsia_cross_cutting_contract_gate_product_roots(
...
function coretsia_cross_cutting_contract_gate_resolve_effective_foundation_reset_tag(
    ...
    $foundationRoot = \rtrim(\str_replace('\\', '/', $foundationRoot), '/');
    ...
    $foundationConfigFile = $foundationRoot . '/config/foundation.php';

    if (!\is_file($foundationConfigFile)) {
        return [$defaultResetTag, []];
    }

    $foundationConfigFile = $repository->resolveExistingFile($foundationConfigFile);

    if (!\is_readable($foundationConfigFile)) {
        return [$defaultResetTag, []];
    }

    $source = DeterministicFile::readTextNormalizedEol($foundationConfigFile);
