...
final class DeterministicFile
{
    ...
    private static function ensureParentDirectory(string $path): void
    {
        ...
        try {
            ...
            // Validate the resulting state below.
        }
    ...
    /**
     * Unique same-directory temp path so final rename stays on the same filesystem.
     */
    private static function atomicTempPath(string $path): string
    {
        ...
        $tmpBase = '.' . $base . '.coretsia-tmp-' . bin2hex(random_bytes(8));

        if ($dir === '' || $dir === '.') {
            return $tmpBase;
        }

        return rtrim($dir, '/\\') . DIRECTORY_SEPARATOR . $tmpBase;
    }
