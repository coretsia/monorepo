...
final class DeptracGenerationContractTest extends ToolContractTestCase
{
    public function testGeneratedDeptracYamlAndGraphArtifactsMatchCanonicalFixture(): void
    {
        ...
        $out = $sandbox . '/tools/testing/deptrac.yaml';
        $allowlist = $sandbox . '/tools/testing/deptrac.allowlist.yaml';
        $artifactsDir = $sandbox . '/var/arch';
    ...
    public function testCheckIgnoresMissingGraphArtifactsAndDoesNotMaterializeThem(): void
    {
        $fixtureRel = 'Deptrac/dependency_graph_ok.php';
        $sandbox = $this->createDeptracSandboxFromDependencyGraphFixture($fixtureRel);
        ...
        [$applyCode, $applyOutput] = $this->runDeptracGenerate($sandbox, [
            '--out',
            $out,
            '--allowlist',
            $allowlist,
        ...
        self::assertSame(0, $applyCode, "Expected deptrac generation to pass.\nOutput:\n" . $applyOutput);

        $this->removeDir($artifactsDir);
        self::assertDirectoryDoesNotExist($artifactsDir);
        ...
        self::assertSame(0, $checkCode, "Expected deptrac check to ignore graph artifacts.\nOutput:\n" . $checkOutput);
        self::assertSame('', $checkOutput);
        self::assertDirectoryDoesNotExist($artifactsDir);
    }
    ...
    public function testCanonicalGeneratorRejectsUnsafeSsotDependencyIdentifiersDeterministicallyWithoutLeak(): void
    {
        foreach (
            [
                'backslash' => 'core\\pkg-b',
                'posix-home-segment' => 'core/home/user',
            ...
            $dependencyTable = $sandbox . '/docs/architecture/DEPENDENCIES.md';
            $out = $sandbox . '/tools/testing/deptrac.yaml';
            $allowlist = $sandbox . '/tools/testing/deptrac.allowlist.yaml';
            $artifactsDir = $sandbox . '/var/arch';
            ...
            self::assertStringContainsString(
                'CORETSIA_DEPTRAC_SSOT_RULESET_MISSING',
                $firstOutput,
            );
