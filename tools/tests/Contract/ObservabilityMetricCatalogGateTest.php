...
final class ObservabilityMetricCatalogGateTest extends TestCase
{
    ...
    public function testObservabilityCatalogGate(callable $arrange, bool $shouldPass, array $expectedDiagnostics): void
    {
        ...
        try {
            ...
            self::mkdirp($repoRoot . '/packages/core/foundation/src');
    ...
    {
        self::writeFile(
            $root . '/packages/core/foundation/src/' . $file,
            self::outdent($code) . "\n",
        );
    }
    ...
    {
        $gateTarget = $repoRoot . '/tools/gates/observability_metric_catalog_gate.php';
        self::writeFile($gateTarget, (string) file_get_contents(self::gatePath()));

        self::writeFile(
            $repoRoot . '/composer.json',
            "{\n"
            . "  \"name\": \"coretsia/gate-fixture\",\n"
            . "  \"type\": \"project\"\n"
            . "}\n",
        );

        self::writeFile(
            $repoRoot . '/packages/core/foundation/composer.json',
            "{\n"
            . "  \"name\": \"coretsia/core-foundation\",\n"
            . "  \"type\": \"library\"\n"
            . "}\n",
        );

        $supportSourceDir = realpath(__DIR__ . '/../../support');

        if (!is_string($supportSourceDir) || $supportSourceDir === '') {
            self::fail('Support source directory cannot be resolved.');
        }

        $supportFiles = glob($supportSourceDir . '/*.php');

        if ($supportFiles === false || $supportFiles === []) {
            self::fail('Support source files cannot be resolved.');
        }

        sort($supportFiles, SORT_STRING);

        self::mkdirp($repoRoot . '/tools/support');

        foreach ($supportFiles as $supportFile) {
            if (!is_string($supportFile)) {
                continue;
            }

            $target = $repoRoot
                . '/tools/support/'
                . basename($supportFile);

            if (!@copy($supportFile, $target)) {
                self::fail(
                    'Failed to copy support file: '
                    . basename($supportFile),
                );
            }
        }
    ...
    foreach (
        glob(__DIR__ . '/../tools/support/*.php') ?: []
        as $path
    ) {
