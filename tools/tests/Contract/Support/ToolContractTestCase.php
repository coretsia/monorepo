...
abstract class ToolContractTestCase extends TestCase
{
    ...
    protected function fixturePath(string $relativePath): string
    {
        ...
        return $this->repoRoot() . '/tools/tests/Fixtures/' . $relativePath;
    }
    ...
    {
        return $this->runPhp(
            $this->repoRoot() . '/tools/build/deptrac_generate.php',
    ...
    {
        return $this->runPhp(
            $this->repoRoot() . '/tools/build/sync_composer_repositories.php',
            array_merge(['--repo-root', $repoRoot], $args),
            $repoRoot,
        );
    }
    ...
    protected function createWorkspaceSandbox(string $fixtureName): string
    {
        ...
        $this->removeDir($sandbox . '/var');

        self::assertFileExists($sandbox . '/composer.json');
        self::assertFileExists($sandbox . '/packages/framework/composer.json');
        self::assertFileExists($sandbox . '/packages/applications/skeleton/composer.json');
    ...
    protected function createDeptracSandboxFromDependencyGraphFixture(string $fixtureRelativePath): string
    {
        ...
        $this->ensureDir($sandbox . '/docs/architecture');
        $this->ensureDir($sandbox . '/packages');
        $this->ensureDir($sandbox . '/tools/testing');
        $this->ensureDir($sandbox . '/var/arch');
        ...
        $this->writeDependencyTable($sandbox . '/docs/architecture/DEPENDENCIES.md', $packages);
        ...
        foreach ($packages as $packageId => $package) {
            ...
            $packageRoot = $sandbox . '/packages/' . $resolvedPackageId;
    ...
    private function writeDependencyTable(string $path, array $packages): void
    {
        ...
        $lines[] = '';
        $lines[] = '## 4) Canonical direct dependency matrix (MUST)';
        $lines[] = '';
        $lines[] = '| package_id | depends_on | notes |';
        $lines[] = '|---|---|---|';
    ...
    protected function assertWorkspaceComposerFilesMatchExpectedFixtures(string $sandbox, string $fixtureName): void
    {
        $pairs = [
            ...
            '/packages/framework/composer.json' => '/expected_composer_framework.json',
            '/packages/applications/skeleton/composer.json' => '/expected_composer_skeleton.json',
        ];
