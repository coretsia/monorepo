...
final class ComposerAuditGateTest extends TestCase
{
    ...
    public function testAdvisoryFoundFailsDeterministically(): void
    {
        ...
        $fakeComposer = $this->writeFakeComposer(
            $repoRoot,
            [
                'root' => $this->fixturePath('audit_with_advisories.json'),
            ],
            [
                'root' => 0,
            ],
        ...
        self::assertSame(
            [
                ...
                'root:vendor/package:PKSA-abcd-efgh-ijkl',
            ],
            $lines,
        );
    ...
    private function runGate(array $args): array
    {
        ...
        $gate = $repoRoot . '/tools/gates/composer_audit_gate.php';
    ...
    /**
     * @param array{root:string} $fixtureByRoot
     * @param array{root:int} $exitByRoot
...
PHP,
    ...
    private function createFixtureRepoRoot(string $prefix): string
    {
        ...
        foreach (['packages', 'tools', 'vendor'] as $directory) {
...
JSON,
    ...
    {
        return $this->repoRoot() . '/tools/tests/Fixtures/ComposerAudit/' . $name;
    }
