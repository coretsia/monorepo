...
final class DtoGateAggregateRunnerTest extends ToolContractTestCase
{
    ...
    public function testAggregateRunnerStopsOnFirstFailureAndPassesOutputThroughUnchanged(): void
    {
        ...
        $expectedOutput = "CORETSIA_FAKE_DTO_FAILURE\n" . "packages/core/demo/src/BrokenDto.php: fake_reason\n";

        $this->withTemporaryDtoSubGates(
            [
                'dto_shape_gate.php' => $this->passingSubGate($logPath, 'shape'),
                'dto_marker_consistency_gate.php' => $this->failingSubGate($logPath, 'marker', $expectedOutput, 37),
                'dto_no_logic_gate.php' => $this->passingSubGate($logPath, 'no-logic'),
            ],
    ...
    public function testAggregateRunnerFailsWhenListedSpecializedGateIsMissing(): void
    {
        ...
        $this->withTemporaryDtoSubGates(
            ...
            function () use ($logPath): void {
                ...
                self::assertSame(
                    "CORETSIA_DTO_GATE_FAILED\n" . "tools/gates/dto_no_logic_gate.php: dto_sub_gate_missing\n",
                    $output,
                );
                self::assertSame("shape\nmarker\n", $this->readBytes($logPath));
            },
    ...
    {
        return $this->runPhp(
            $this->repoRoot() . '/tools/gates/dto_gate.php',
            [],
            $this->repoRoot(),
        );
    }
    ...
    {
        $gateDir = $this->repoRoot() . '/tools/gates';
        $backups = [];
    ...
    private function subGateScript(string $logPath, string $token, string $output, int $exitCode): string
    {
        return "<?php\n\n"
            . "declare(strict_types=1);\n\n"
            . "file_put_contents(" . var_export($logPath, true) . ", " . var_export(
                $token . "\n",
                true,
            ) . ", FILE_APPEND);\n"
