...
final class SecretLeakageGateTest extends TestCase
{
    ...
    private function runGateWithFixture(string $fixtureName, int $gitleaksExitCode): array
    {
        ...
        $sourceFixturePath = $repoRoot . '/tools/tests/Fixtures/Gitleaks/' . $fixtureName;

        self::assertFileExists($sourceFixturePath);

        $scanRoot = $this->makeFixtureScanRoot();
        $fixturePath = $scanRoot . '/' . $fixtureName;

        self::assertNotFalse(
            \copy($sourceFixturePath, $fixturePath),
        );

        try {
            $gate = $repoRoot . '/tools/gates/secret_leakage_gate.php';
    ...
    {
        $root = $this->repoRoot() . '/var/phpunit/secret-leakage-gate-' . \bin2hex(\random_bytes(8));
