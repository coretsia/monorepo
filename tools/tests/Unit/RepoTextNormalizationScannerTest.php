...
final class RepoTextNormalizationScannerTest extends TestCase
{
    ...
    {
        $repository = RepositoryContext::fromRepoRoot($root);

        return RepoTextNormalizationScanner::scan(
            $repository,
            $root,
        );
    }
    ...
    private static function createTempRoot(): string
    {
        ...
        self::mkdirp($dir . '/packages');
        self::mkdirp($dir . '/tools');
        self::writeFile(
            $dir . '/composer.json',
            "{\n"
            . "  \"name\": \"coretsia/repo-text-normalization-fixture\",\n"
            . "  \"type\": \"project\"\n"
            . "}\n",
        );
